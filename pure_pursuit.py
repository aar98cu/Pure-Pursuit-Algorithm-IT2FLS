...
with dpg.window(label="Pure Pursuit Algorithm Simulation", tag="window"):
    ...
    with dpg.group(horizontal=True):
        ...
        with dpg.group(horizontal=False, tag="left_panel", width=int(0.2 * width), height=height):
            ...
            dpg.add_combo(velocity_control, default_value="Select speed control", width=-1, callback=update_velocity_control, tag="speed")

            # Checkbox to toggle noise simulation
            dpg.add_checkbox(label="Simulate with noise", tag="noise_checkbox", default_value=False, callback=toggle_noise_settings)

            # Dropdown for selecting the velocity control algorithm
            dpg.add_combo(steering_mode, default_value="Select steering mode", width=-1, callback=update_steering_mode, tag="steering", show=False)
            ...
            dpg.add_text("Max steering delta in degrees", tag="max_steering_delta_text", show=False)
            dpg.add_input_float(tag="max_steering_delta", default_value=math.degrees(max_delta_steering), format="%.0f", width=200, max_value=35, min_value=1, min_clamped=True, max_clamped=True, step=1, show=False)

            # Input for setting the maximum speed delta in meters per second
            dpg.add_text("Max speed delta in m/s", tag="max_speed_delta_text", show=False)
            dpg.add_input_float(tag="max_speed_delta", default_value=max_delta_speed, format="%.2f", width=200, max_value=10, min_value=0.01, min_clamped=True, max_clamped=True, step=0.05, show=False)
